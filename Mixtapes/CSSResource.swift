...
public class CSSResource: Resource {
    init(index: Index) {
        super.init(url: URL(fileURLWithPath: index.url.relativePath.replacingOccurrences(of: ".txt", with: ".css"), relativeTo: index.url.baseURL!), data: _css.data(using: .utf8)!, resources: [Resource(url: URL(fileURLWithPath: "gaegu.woff", relativeTo: index.url.baseURL!), data: Data(base64Encoded: _gaegu)!)])
    }
}

private let _gaegu: String = """
