...
public class HTML: Resource {
    public init(index: Index) {
        func header(index: Index, item: Item? = nil) -> String {
            var string: String = ""
            string += "<!DOCTYPE html>\n"
            ...
            string += "<meta name=\"viewport\" content=\"initial-scale=1.0, viewport-fit=cover\">\n"
            string += "<link rel=\"alternate\" href=\"\(URL(fileURLWithPath: "index.rss", relativeTo: index.homepage).absoluteString)\" type=\"application/rss+xml\" title=\"RSS\">\n"
            if let icon: Resource = index.icon {
                string += "<link rel=\"apple-touch-icon\" href=\"\(icon.url.relativePath)\">\n"
            }
            string += "<link rel=\"stylesheet\" href=\"stylesheet.css\">\n"
            string += "<header>\n"
            string += "    <nav><a href=\"index.rss\"><img src=\"rss.svg\" alt=\"RSS\"></a></nav>\n"
            if let _: Item = item {
                string += "    <h1><a href=\"index.html\">\(index.title)</a></h1>\n"
            } else {
                string += "    <h1>\(index.title)</h1>\n"
            }
            string += "    <hr>\n"
            string += "</header>\n"
            return string
        ...
        func footer(index: Index) -> String {
            var string: String = ""
            string += "<footer>\n"
            if let author: Author = index.author {
                string += "    <p><a href=\"\(author.url.absoluteString)\">\(author.description)</a></p>\n"
            }
            string += "</footer>"
            return string
        }
        ...
        var resources: [Resource] = [
            Resource(url: URL(fileURLWithPath: "rss.svg", relativeTo: index.url), data: RSSIcon_Data),
            Resource(url: URL(fileURLWithPath: "gaegu.woff", relativeTo: index.url), data: GaeguFont_Data),
            Resource(url: URL(fileURLWithPath: "stylesheet.css", relativeTo: index.url), data: Stylesheet_Data)
        ]
        for item in index.items {
            var string: String = ""
            string += header(index: index, item: item)
            string += "<main>\n"
            string += "    <img src=\"\(item.image.relativePath)\">\n"
            string += "    <audio preload=\"metadata\" controls><source src=\"\(item.attachment.url.relativePath)\" type=\"\(item.attachment.mimeType)\"></audio>\n"
            string += "    <table>\n"
            string += "        <tr>\n"
            string += "            <td><a href=\"\(item.id).html\">\(item.title) - \(item.summary)</a></td>\n"
            string += "            <td><time>\(item.attachment.asset.duration.description)</time></td>\n"
            string += "        </tr>\n"
            string += "    </table>\n"
            if !item.attachment.asset.chapters.isEmpty {
                ...
                for chapter in item.attachment.asset.chapters {
                    ...
                    string += "            <td><time>\(chapter.duration.lowerBound.description)</time></td>\n"
                    string += "        </tr>\n"
                }
                string += "    </table>\n"
                string += "    <hr>\n"
            ...
            string += "</main>\n"
            string += footer(index: index)
            resources.append(Resource(url: URL(fileURLWithPath: "\(item.id).html", relativeTo: index.url), data: string.data(using: .utf8)!))
        }
        var string: String = ""
        string += header(index: index)
        string += "<main>\n"
        for item in index.items {
            string += "    <table>\n"
            string += "        <tr>\n"
        ...
        string += "</main>\n"
        string += footer(index: index)
        super.init(url: URL(fileURLWithPath: "index.html", relativeTo: index.url), data: string.data(using: .utf8)!, resources: resources)
    }
}

private let RSSIcon_Data: Data = """
...
private let GaeguFont_Data: Data = Data(base64Encoded: """
